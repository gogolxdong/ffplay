    ...
    if (!*texture || SDL_QueryTexture(*texture, &format, &access, &w, &h) < 0 || new_width != w || new_height != h || new_format != format)
    {
        ...
        if (init_texture)
        {
            if (SDL_LockTexture(*texture,
                                ((void *)0), &pixels, &pitch) < 0)
                return -1;
            memset(pixels, 0, pitch * new_height);
            SDL_UnlockTexture(*texture);
        }
        av_log(
            ((void *)0), 40, "Created %dx%d texture with %s.\n", new_width, new_height, SDL_GetPixelFormatName(new_format));
    }
    return 0;
}
...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx)
{
    ...
    case SDL_PIXELFORMAT_UNKNOWN:
        *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
        ...
        {
            av_log(
                ((void *)0), 8, "Cannot initialize the conversion context\n");
            ret = -1;
        }
        break;
    case SDL_PIXELFORMAT_IYUV:
        if (frame->linesize[0] > 0 && frame->linesize[1] > 0 && frame->linesize[2] > 0)
        {
            ret = SDL_UpdateYUVTexture(*tex,
                                       ((void *)0), frame->data[0], frame->linesize[0],
        ...
        {
            ret = SDL_UpdateYUVTexture(*tex,
                                       ((void *)0), frame->data[0] + frame->linesize[0] * (frame->height - 1), -frame->linesize[0],
        ...
        {
            av_log(
                ((void *)0), 16, "Mixed negative and positive linesizes are not supported.\n");
            return -1;
        }
        break;
    default:
...
static void video_image_display(VideoState *is)
{
    ...
    SDL_RenderCopyEx(renderer, is->vid_texture,
                     ((void *)0), &rect, 0,
                     ((void *)0), vp->flip_v ? SDL_FLIP_VERTICAL : 0);
...
static void video_audio_display(VideoState *s)
{
    ...
    else
    {
        ...
        {
            av_log(
                ((void *)0), 16, "Failed to allocate buffers for RDFT, switching to waves display\n");
...
static double compute_target_delay(double delay, VideoState *is)
{
    ...
    av_log(
        ((void *)0), 56, "video: delay=%0.3f A-V=%f\n",
        delay, -diff);
    return delay;
}
...
#define INSERT_FILT(name, arg)                                                                                       \
    ...
    if (autorotate)
    {
        ...
        if (fabs(theta - 90) < 1.0)
        {
            ...
            {
                AVFilterContext *filt_ctx;
                ret = avfilter_graph_create_filter(&filt_ctx, avfilter_get_by_name("transpose"), "ffplay_"
                                                                                                 "transpose",
                                                   "clock",
                                                   ((void *)0), graph);
                if (ret < 0)
        ...
        else if (fabs(theta - 180) < 1.0)
        {
            ...
            {
                AVFilterContext *filt_ctx;
                ret = avfilter_graph_create_filter(&filt_ctx, avfilter_get_by_name("hflip"), "ffplay_"
                                                                                             "hflip",
                                                   ((void *)0),
                                                   ((void *)0), graph);
                if (ret < 0)
            ...
            {
                AVFilterContext *filt_ctx;
                ret = avfilter_graph_create_filter(&filt_ctx, avfilter_get_by_name("vflip"), "ffplay_"
                                                                                             "vflip",
                                                   ((void *)0),
                                                   ((void *)0), graph);
                if (ret < 0)
        ...
        else if (fabs(theta) > 1.0)
        {
            ...
            {
                AVFilterContext *filt_ctx;
                ret = avfilter_graph_create_filter(&filt_ctx, avfilter_get_by_name("rotate"), "ffplay_"
                                                                                              "rotate",
                                                   rotate_buf,
                                                   ((void *)0), graph);
                if (ret < 0)
        ...
        if (got_frame)
        {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0)
            {
                ...
                if (!(af = frame_queue_peek_writable(&is->sampq)))
                    goto the_end;
                af->pts = (frame->pts == ((int64_t)0x8000000000000000ULL)) ? __builtin_nan("")
                                                                           : frame->pts * av_q2d(tb);
...
static int video_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (last_w != frame->width || last_h != frame->height || last_format != frame->format || last_serial != is->viddec.pkt_serial || last_vfilter_idx != is->vfilter_idx)
        {
            ...
            if ((ret = configure_video_filters(graph, is, vfilters_list ? vfilters_list[is->vfilter_idx] : ((void *)0), frame)) < 0)
            {
                SDL_Event event;
        ...
        while (ret >= 0)
        {
            ...
            pts = (frame->pts == ((int64_t)0x8000000000000000ULL)) ? __builtin_nan("")
                                                                   : frame->pts * av_q2d(tb);
    ...
    if (get_master_sync_type(is) != AV_SYNC_AUDIO_MASTER)
    {
            ...
            fabs(diff) < 10.0)
        {
            ...
            else
            {
                ...
                av_log(
                    ((void *)0), 56, "diff=%f adiff=%f sample_diff=%d apts=%0.3f %f\n",
...
static int audio_decode_frame(VideoState *is)
{
    ...
    if (is->swr_ctx)
    {
        ...
        int out_size = av_samples_get_buffer_size(
            ((void *)0), is->audio_tgt.channels, out_count, is->audio_tgt.fmt, 0);
        int len2;
        if (out_size < 0)
        {
            av_log(
                ((void *)0), 16, "av_samples_get_buffer_size() failed\n");
            return -1;
        }
        if (wanted_nb_samples != af->frame->nb_samples)
        {
            ...
            {
                av_log(
                    ((void *)0), 16, "swr_set_compensation() failed\n");
                return -1;
            }
        }
        ...
        {
            av_log(
                ((void *)0), 16, "swr_convert() failed\n");
            return -1;
        }
        if (len2 == out_count)
        {
            av_log(
                ((void *)0), 24, "audio buffer is probably too small\n");
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    {
        av_log(
            ((void *)0), 16, "Invalid sample rate or channel count!\n");
        return -1;
    }
    ...
    while (!(audio_dev = SDL_OpenAudioDevice(
                 ((void *)0), 0, &wanted_spec, &spec, 0x00000001 | 0x00000004)))
    {
        av_log(
            ((void *)0), 24, "SDL_OpenAudio (%d channels, %d Hz): %s\n",
    ...
    audio_hw_params->frame_size = av_samples_get_buffer_size(
        ((void *)0), audio_hw_params->channels, 1, audio_hw_params->fmt, 1);
    audio_hw_params->bytes_per_sec = av_samples_get_buffer_size(
        ((void *)0), audio_hw_params->channels, audio_hw_params->freq, audio_hw_params->fmt, 1);
...
static int read_thread(void *arg)
{
    ...
    if (start_time != ((int64_t)0x8000000000000000ULL))
    {
        ...
        ret = avformat_seek_file(ic, -1,
                                 (-9223372036854775807LL - 1), timestamp,
                                 9223372036854775807LL, 0);
        if (ret < 0)
        {
            av_log(
                ((void *)0), 24, "%s: could not seek to position %0.3f\n",
    ...
    for (i = 0; i < AVMEDIA_TYPE_NB; i++)
    {
        ...
        {
            av_log(
                ((void *)0), 16, "Stream specifier %s does not match any %s stream\n", wanted_stream_spec[i], av_get_media_type_string(i));
    ...
    {
        av_log(
            ((void *)0), 8, "Failed to open file '%s' or configure filtergraph\n",
            is->filename);
        ret = -1;
        goto fail;
    }
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (startup_volume < 0)
        av_log(
            ((void *)0), 24, "-volume=%d < 0, setting to 0\n", startup_volume);
    if (startup_volume > 100)
        av_log(
            ((void *)0), 24, "-volume=%d > 100, setting to 100\n", startup_volume);
...
the_end:
    ...
    av_log(
        ((void *)0), 32, "Switch %s stream from #%d to #%d\n",
...
static void seek_chapter(VideoState *is, int incr)
{
    ...
    if (i >= is->ic->nb_chapters)
        return;
    av_log(
        ((void *)0), 40, "Seeking to chapter %d.\n", i);
...
{
    av_log(
        ((void *)0), 24, "Option -s is deprecated, use -video_size.\n");
    return opt_default(
        ((void *)0), "video_size", arg);
}
...
{
    av_log(
        ((void *)0), 24, "Option -pix_fmt is deprecated, use -pixel_format.\n");
    return opt_default(
        ((void *)0), "pixel_format", arg);
}
...
static int opt_sync(void *optctx, const char *opt, const char *arg)
{
    ...
    {
        av_log(
            ((void *)0), 16, "Unknown value for %s: %s\n", opt, arg);
        exit(1);
    }
    return 0;
}
...
{
    av_log(
        ((void *)0), 32, "Simple media player\n");
    av_log(
        ((void *)0), 32, "usage: %s [options] input_file\n", program_name);
    av_log(
        ((void *)0), 32, "\n");
}
...
int main(int argc, char **argv)
{
    ...
    parse_options(
        ((void *)0), argc, argv, options, opt_input_file);
    if (!input_filename)
    {
        show_usage();
        av_log(
            ((void *)0), 8, "An input file must be specified\n");
        av_log(
            ((void *)0), 8,
    ...
    {
        av_log(
            ((void *)0), 8, "Could not initialize SDL - %s\n", SDL_GetError());
        av_log(
            ((void *)0), 8, "(Did you set the DISPLAY variable?)\n");
        exit(1);
    }
    ...
    if (!display_disable)
    {
        ...
        if (window)
        {
            ...
            {
                av_log(
                    ((void *)0), 24, "Failed to initialize a hardware accelerated renderer: %s\n", SDL_GetError());
            ...
            if (renderer)
            {
                if (!SDL_GetRendererInfo(renderer, &renderer_info))
                    av_log(
                        ((void *)0), 40, "Initialized %s renderer.\n", renderer_info.name);
            }
        }
        if (!window || !renderer || !renderer_info.num_texture_formats)
        {
            av_log(
                ((void *)0), 8, "Failed to create window or renderer: %s", SDL_GetError());
            do_exit(
                ((void *)0));
        }
    }
    ...
    {
        av_log(
            ((void *)0), 8, "Failed to initialize VideoState!\n");
        do_exit(
            ((void *)0));
    }
    event_loop(is);
